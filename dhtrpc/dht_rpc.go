...
//nolint:errcheck // The Write call on the hash instance cannot error
...
func (d *DHT) rotateSecrets(ctx context.Context) {
	...
	for {
		...
		case <-ticker.C:
			...
			if n, err := rand.Read(d.secrets[0]); err != nil || n != len(d.secrets[0]) {
				panic(fmt.Errorf("read %d/%d random bytes secret: %w", n, len(d.secrets[0]), err))
			}
			d.lock.Unlock()
		}
	}
...
func (d *DHT) forwardRequest(from *udprequest.PeerRequest, req *request) {
	...
	if buf, err := proto.Marshal(req); err == nil {
		d.socket.ForwardRequest(from, to, buf) //nolint:errcheck
	}
}
...
func (d *DHT) HandleUDPRequest(p *udprequest.PeerRequest, reqBuf []byte) {
	...
	if res != nil {
		...
		if buf, err := proto.Marshal(res); err == nil {
			d.socket.Respond(p, buf) //nolint:errcheck
		}
	}
}
